...
# Obtaining employees from BE and Removing PSD people
df_BE = df_imported.loc[df_imported["Business Unit (Mapped)"] == "Business Enablement"]
df_BE = df_BE.loc[df_BE["Interviewer Vertical"] != "PEOPLE STRATEGY & DEVELOPMENT"]
...
df_BE_conducted = df_BE.loc[df_BE["Interview Status (Updated)"] == "Conducted"]

# Creating a df of interviews that were cancelled/rescheduled
df_BE_cancelled_and_rescheduled = df_BE[df_BE["Interview Status (Updated)"] == "Cancelled"]

# Creating a df of rescheduled interviews and handling the columns
df_BE_rescheduled = pd.merge(df_BE_cancelled_and_rescheduled, df_BE_conducted, on=["Employee Name",
...
df_BE_cancelled = pd.merge(df_BE_cancelled_and_rescheduled, df_BE_rescheduled,
...
"""
df_final = df_BE_conducted[["Interviewer", "Interviewer Vertical", "Business Unit (Mapped)"]].reset_index()
...
df_rescheduled_temp = df_BE_rescheduled[["Interviewer", "Interviewer Vertical", "Business Unit (Mapped)"]].reset_index()
...
df_cancelled_temp = df_BE_cancelled[["Interviewer", "Interviewer Vertical", "Business Unit (Mapped)"]].reset_index()
...
df_feedback_one = df_BE.loc[df_BE["Feedback Interval"] == "Feedback within 1 working day"]
...
df_feedback_more_than_one = df_BE.loc[df_BE["Feedback Interval"] == "More than 1 working day for Feedback"]
...
# Importing All Candidates Data For BE
...
# Creating a df of BE members such that each employee's name occurs only once
df_BE_non_PSD = df_BE[["Interviewer"]].reset_index()
df_BE_non_PSD = df_BE_non_PSD.groupby("Interviewer").count().reset_index()
...
df_offered_temp = pd.merge(df_offered_temp, df_BE_non_PSD, on="Interviewer", how="inner")
...
df_BE_rejected = df_BE_new.loc[(df_BE_new["Interview Result/Status"] == "Rejected") |
                               (df_BE_new["Interview Result/Status"] == "Pending for Decision - Rejected")]
df_BE_rejected = df_BE_rejected.loc[df_BE_rejected["Interview Stage Name"] != "First Round"]
df_BE_rejected.reset_index(inplace=True)
df_BE_rejected.drop(columns=["index"], inplace=True)
df_BE_rejected = df_BE_rejected.iloc[:, 0:30]
...
df_BE_rejected_later = df_BE_rejected_later.loc[df_BE_rejected_later["Interview Status (Updated)"] == "Conducted"]
df_BE_rejected_later = df_BE_rejected_later.loc[df_BE_rejected_later["Interview Result/Status"] != "Rejected"]
df_BE_rejected_later = df_BE_rejected_later.loc[df_BE_rejected_later["Interview Result/Status"] != "Pending for Decision - Rejected"]

# Removing Interviewers that are in PSD
df_BE_rejected_later = df_BE_rejected_later.loc[(df_BE_rejected_later["Interviewer Vertical"] != "PEOPLE STRATEGY & DEVELOPMENT")]
df_BE_rejected_later.reset_index(inplace=True)
df_BE_rejected_later.drop(columns=["index"], inplace=True)
df_BE_rejected_later = df_BE_rejected_later.iloc[:, 0:30]
...
df_BE_rejected_later["Rejected In Later Rounds"] = 0
df_BE_rejected_later = df_BE_rejected_later.groupby(["Interviewer"]).count().reset_index()

# Adding to final df
df_final = df_final.merge(df_BE_rejected_later, on="Interviewer", how="outer")
...
df_final.to_excel(r"C:\Users\Diwaker Sinha\Desktop\MPL\Business Enablement.xlsx")
