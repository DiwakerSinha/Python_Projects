...
# Obtaining employees from CEO and Removing PSD people
df_CEO = df_imported.loc[df_imported["Business Unit (Mapped)"] == "CEO Office"]
df_CEO = df_CEO.loc[df_CEO["Interviewer Vertical"] != "PEOPLE STRATEGY & DEVELOPMENT"]

# To prevent runtime suggestions
pd.set_option('mode.chained_assignment', None)

# Filtering Interviews Conducted By Month
df_CEO["Interview Scheduled Date"] = np.datetime_as_string(df_CEO["Interview Scheduled Date"], unit='D')
df_CEO = df_CEO.loc[df_CEO["Interview Scheduled Date"].astype(str).str.contains("2021-06-")]

# Creating df of interviews that were Conducted
df_CEO_conducted = df_CEO.loc[df_CEO["Interview Status (Updated)"] == "Conducted"]

# Creating a df of interviews that were cancelled/rescheduled
df_CEO_cancelled_and_rescheduled = df_CEO[df_CEO["Interview Status (Updated)"] == "Cancelled"]

# Creating a df of rescheduled interviews and handling the columns
df_CEO_rescheduled = pd.merge(df_CEO_cancelled_and_rescheduled, df_CEO_conducted, on=["Employee Name",
                                                                                      "Interview Candidate ID",
                                                                                      "Job Code"], how="inner")
df_CEO_rescheduled["Interview Status (Updated)_x"] = "Rescheduled"
df_CEO_rescheduled = df_CEO_rescheduled.loc[:, 'Employee Name':"Adjusted Rating Date_x"]
df_CEO_rescheduled.columns = [col.replace('_x', '') for col in df_CEO_rescheduled.columns]


# Creating a df of cancelled interviews and handling the columns
df_CEO_cancelled = pd.merge(df_CEO_cancelled_and_rescheduled, df_CEO_rescheduled,
                            on=["Employee Name", "Interview Candidate ID",
                                "Job Code"], how='outer', indicator=True) \
    .query("_merge != 'both'") \
    .drop('_merge', axis=1) \
    .reset_index(drop=True)
df_CEO_cancelled = df_CEO_cancelled.loc[:, 'Employee Name':'Adjusted Rating Date_x']
df_CEO_cancelled.columns = [col.replace('_x', '') for col in df_CEO_cancelled.columns]
df_CEO_cancelled = df_CEO_cancelled.iloc[:, 0:30]


"""
Creating the first instance of the final df and instantiating it with the conducted interviews
"""
df_final = df_CEO_conducted[["Interviewer", "Interviewer Vertical", "Business Unit (Mapped)"]].reset_index()
...
df_rescheduled_temp = df_CEO_rescheduled[["Interviewer", "Interviewer Vertical", "Business Unit (Mapped)"]].reset_index()
...
df_cancelled_temp = df_CEO_cancelled[["Interviewer", "Interviewer Vertical", "Business Unit (Mapped)"]].reset_index()
...
df_feedback_one = df_CEO.loc[df_CEO["Feedback Interval"] == "Feedback within 1 working day"]
...
df_feedback_more_than_one = df_CEO.loc[df_CEO["Feedback Interval"] == "More than 1 working day for Feedback"]
...
# Importing All Candidates Data For CEO
...
# Creating a df of CEO members such that each employee's name occurs only once
df_CEO_non_PSD = df_CEO[["Interviewer"]].reset_index()
df_CEO_non_PSD = df_CEO_non_PSD.groupby("Interviewer").count().reset_index()
...
df_offered_temp = pd.merge(df_offered_temp, df_CEO_non_PSD, on="Interviewer", how="inner")
...
df_CEO_new = pd.read_excel(r"C:\Users\Diwaker Sinha\Desktop\Interviewer R&R.xlsx", sheet_name="Interviews Raw Data")
df_CEO_new = df_CEO_new.loc[df_CEO_new["Business Unit (Mapped)"] == "CEO Office"]


# Making a df of the people that were rejected after the first round
df_CEO_rejected = df_CEO_new.loc[(df_CEO_new["Interview Result/Status"] == "Rejected") |
                                 (df_CEO_new["Interview Result/Status"] == "Pending for Decision - Rejected")]
df_CEO_rejected = df_CEO_rejected.loc[df_CEO_rejected["Interview Stage Name"] != "First Round"]
df_CEO_rejected.reset_index(inplace=True)
df_CEO_rejected.drop(columns=["index"], inplace=True)
df_CEO_rejected = df_CEO_rejected.iloc[:, 0:30]

# Obtaining all the interviews for the people that were eventually rejected
df_CEO_rejected_later = pd.merge(df_CEO_new, df_CEO_rejected, on=["Interview Candidate Name", "Interview Candidate ID",
                                                                  "Job Code"], how="inner")

# Fixing the column names and making the df normal sized (columns)
df_CEO_rejected_later = df_CEO_rejected_later.iloc[:, 0:30]
df_CEO_rejected_later.columns = [col.replace('_x', '') for col in df_CEO_rejected_later.columns]

# Removing the rejected interviews since they are at the last stage and only sorting for interviews that were conducted
df_CEO_rejected_later = df_CEO_rejected_later.loc[df_CEO_rejected_later["Interview Status (Updated)"] == "Conducted"]
df_CEO_rejected_later = df_CEO_rejected_later.loc[df_CEO_rejected_later["Interview Result/Status"] != "Rejected"]
df_CEO_rejected_later = df_CEO_rejected_later.loc[df_CEO_rejected_later["Interview Result/Status"] != "Pending for Decision - Rejected"]

# Removing Interviewers that are in PSD
df_CEO_rejected_later = df_CEO_rejected_later.loc[(df_CEO_rejected_later["Interviewer Vertical"] != "PEOPLE STRATEGY & DEVELOPMENT")]
df_CEO_rejected_later.reset_index(inplace=True)
df_CEO_rejected_later.drop(columns=["index"], inplace=True)
df_CEO_rejected_later = df_CEO_rejected_later.iloc[:, 0:30]

# Only keeping the interviewer name
df_CEO_rejected_later = df_CEO_rejected_later["Interviewer"].to_frame().reset_index()
df_CEO_rejected_later.drop(columns=["index"], inplace=True)
df_CEO_rejected_later = df_CEO_rejected_later.iloc[:, 0:30]

# Counting how many instances of Rejected in Later Rounds are present for a given interviewer
df_CEO_rejected_later["Rejected In Later Rounds"] = 0
df_CEO_rejected_later = df_CEO_rejected_later.groupby(["Interviewer"]).count().reset_index()

# Adding to final df
df_final = df_final.merge(df_CEO_rejected_later, on="Interviewer", how="outer")
...
df_final.to_excel(r"C:\Users\Diwaker Sinha\Desktop\MPL\CEO Office.xlsx")
